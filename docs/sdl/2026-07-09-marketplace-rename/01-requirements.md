...
Rename the Claude Code plugin marketplace from `savioke` to `relay` following the company rename. The GitHub org remains `savioke` (the desired name is taken; renaming an org is disruptive), so this cycle changes only the marketplace identity: `name`, `displayName`, `owner.name`, and description in `.claude-plugin/marketplace.json`, the `sdl@savioke` → `sdl@relay` install/update references in `scripts/install.sh` output and docs, and one prose mention of the company in `docs/admin-setup.md`. The marketplace name is an arbitrary identifier — nothing binds it to the GitHub org.

## Scope

In scope: marketplace identity fields in `.claude-plugin/marketplace.json`; `sdl@relay` and `/plugin marketplace update relay` strings in `scripts/install.sh`, `docs/developer-guide.md`, and `Plan.md`; company-name prose in `docs/admin-setup.md`.
...
- The new name `relay` must be a valid marketplace identifier (kebab-case, not an Anthropic-reserved name).
- Docs must state the correct post-rename update command (`/plugin marketplace update relay`); the rename is a breaking change for registered installs, migrated manually by the sole existing user.
