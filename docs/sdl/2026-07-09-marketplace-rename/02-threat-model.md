...
- **`scripts/install.sh`** — workstation setup script (baseline:B3). Edits are string-only: the install target `sdl@savioke` → `sdl@relay` and printed footer text. The legacy-marketplace auto-removal considered during development was reverted before commit.
...
- **Component / flow:** existing developer machines with the marketplace registered as `savioke`, after `git pull` renames it to `relay` in the underlying clone.
...
- **Mitigation:** manual migration by the sole existing user: `claude plugin uninstall sdl@savioke`, `claude plugin marketplace remove savioke`, re-run `install.sh` (registers `relay`, installs `sdl@relay`). Fresh installs are unaffected.
- **Mitigation type:** corrective
- **Defense in depth notes:** `install.sh`'s printed footer and `developer-guide.md` now name `relay` in the update command, so a half-migrated install fails loudly at the documented step rather than drifting unnoticed.
...
- **2026-06-11-fix-update-instructions** wrote no numbered threats; its concern (documented update procedure must be accurate) carries into this diff — the documented command is now `/plugin marketplace update relay`, matching the renamed manifest.
...
- Marketplace-name squatting/spoofing of `relay` — marketplace names are per-user local identifiers in Claude Code, not a global registry; there is nothing to squat. Reserved-name collision checked (not an Anthropic-reserved name).
