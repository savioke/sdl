...
- **Bounded, timed, non-raising network I/O** for the one external fetch (`check_release.py:129-147`); no other network access in either tool.
...
- **Correct diff range:** shipped-change detection uses three-dot (`merge-base..HEAD`, `check_release.py:114-121`), matching `validate.py`; a two-dot comparison would attribute `main`'s post-branch-point changes to the PR. Caught in functional testing and regression-tested.
