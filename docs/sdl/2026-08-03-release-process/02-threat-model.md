...
- **Mitigation:** HTTPS with default certificate verification; 30-second timeout; read capped at 1 MB before parsing (`check_release.py`, `fetch_manifest`); every failure mode — HTTP error, transport error, oversize, undecodable bytes, malformed JSON — returns an error value rather than raising, and the caller downgrades it to a note (SR-8). Parsing successfully is not the same as being well-shaped: `check_manifest` type-checks the top level, the `plugins` array, each entry, and `source` before reading through them, so a syntactically valid manifest of the wrong shape is reported as drift rather than raised as a traceback. Only a manifest that is fetched *and* parsed *and* disagrees fails the build.
