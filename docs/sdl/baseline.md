...
- `scripts/install.sh` — installs the Claude Code and Codex plugins and symlinks skills for Copilot; `scripts/sync-to-repo.sh` — onboards a consumer repo.
- `scripts/release.sh` — cuts a release; `scripts/check_release.py` — detects release drift in CI.
- Distributed by tagged release (`docs/releasing.md`), never from `main`: consumer CI resolves the moving major alias, `@v2` today; Claude Code and Codex developers get the `sdl@relay` plugin, whose host-specific catalogs in `savioke/relay-plugin-marketplace` pin the same immutable `vX.Y.Z` tag; Copilot/other-agent developers use per-developer clones at `~/.sdl-governance` (skills via symlink), which track `main` and are the one channel not fed by a tag.
...
- **This repo → developer workstation (Claude Code and Codex).** The `sdl@relay` plugin is fetched from an immutable release tag as directed by the two host-specific catalogs in `savioke/relay-plugin-marketplace`. Crossing point: each catalog's `source` (url/path/ref) — integrity assets that live outside this repo and its SDL gate.
...
- the marketplace catalogs in `savioke/relay-plugin-marketplace` (external to this repo — see B7)
...
- No CI job holds a credential that can write a tag or the marketplace catalogs — CI detects release drift, the maintainer performs releases.
...
| B3 | Workstation scripts run with developer privileges: `install.sh` invokes the Claude Code and Codex plugin installers and writes into `~/.copilot`; `sync-to-repo.sh` writes into target repos. | medium | accept | Small, auditable, `shellcheck`-gated. Plugin installation remains explicit and uses the public Relay marketplace. Revisit if the scripts gain privileged operations or another download source. |
...
| B7 | Marketplace catalogs live outside this repo's gates: `savioke/relay-plugin-marketplace` has no SDL cycle, CI, or validator, yet their `source` fields decide what code `sdl@relay` delivers to Claude Code and Codex developer machines. A malicious or mistaken edit can repoint either host at arbitrary code or make the hosts run different releases. Partially mitigated 2026-08-03 (cycle 2026-08-03-release-process) and 2026-08-22 (cycle 2026-08-22-codex-support): both `source.ref` values pin the same immutable release tag instead of `main`; `release.sh` updates them together; and `check_release.py` fails this repo's CI daily if either catalog stops naming the released tag. Write access to that repo remains the exposure. | medium | accept | Org-member-only write access; drift is detected within a day. Revisit (branch protection, SDL gating) when the marketplace gains more plugins, maintainers, or users. |
