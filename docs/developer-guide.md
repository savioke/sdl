...
Installs skills globally for Claude Code and Copilot. To update: `cd ~/.sdl-governance && git pull`. Copilot picks up the new skills immediately (they are symlinked); **Claude Code does not** — its plugin marketplace is a local clone that does not auto-refresh, so after pulling also run `/plugin marketplace update relay` and reload when prompted.
